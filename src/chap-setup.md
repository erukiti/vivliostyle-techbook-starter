...
ひとつはフォントの埋め込みです。Vivliostyleでは多くのフォントは問題なく埋め込みが自動的にされているようですが、特殊な文字などでType 3フォントというPostScript由来の特殊なフォントが使われることがあるようですが、これが悪さをすることがあり、印刷所としても苦労しているようです。じつはこの本を入稿して安心してたら、オペレーターの人から電話がかかってきて、顔が真っ青になったところです。

たとえば、完全に大丈夫なパターンとしては、すべての文字をアウトライン化するという方法ですが、これを行うと文字が太くなったり、変なことになることがあります。この本で実際に試したところ、意味不明なところで太くなったり、なぜかベクターフォントではなくラスターフォントみたいな風になったりするという謎の挙動を示してしまって、実用に耐えないと判断しました<span class="footnote">ただ太くなるだけなら、PDFをディスプレイ上で確認する分にはアウトですが、印刷所の高解像度印刷機だと誤差の範囲に収まるから大丈夫というアドバイスをいただいたんですが、検証する時間などが足りないのと、ラスターフォントみたいになっちゃう現象はさすがに無理だと判断しました。</span>。
...
この程度なら、必ずしも壊れるとは限らないため、これなら別にいいかとなっているところです。

この問題に際して一部の原稿を削りました<span class="footnote">pdffontsと言うコマンドをつかって印刷所受付可能なPDFかどうかを確認しようという内容でした。失敗してんじゃんよ。とほほ</span>。
</div>
